...
async function getUserInfo(username) {
  try {
    const port = 3000; 
    const response = await fetch(`http://localhost:${port}/api/user/${username}`, {    
      method: 'GET',
      headers: {
...
class App {
  ...
  constructor() {
    ...
    document.querySelector('.form').addEventListener('submit', this._newMarker.bind(this));
  }
  ...
  _hideForm() {
    ...
    setTimeout(() => (document.querySelector('.form').style.display = 'grid'), 1000);
  }
